...
class FranjaEstaCerradaTest(unittest.TestCase):
    ...
    def test_al_iniciar_sigue_abierta_antes_del_inicio(self):
        slot = self._slot(cierre_modo="al_iniciar")
        # 1 minuto antes de hora_inicio (14:00) sigue abierta
        self.assertFalse(franja_esta_cerrada(slot, datetime(2026, 8, 17, 13, 59)))
        # Al llegar hora_inicio cierra (por el guard superior ahora>=inicio_dt)
        self.assertTrue(franja_esta_cerrada(slot, datetime(2026, 8, 17, 14, 0)))
