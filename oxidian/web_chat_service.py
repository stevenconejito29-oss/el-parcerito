...
def add_message(conversation, sender: str, body: str, *, agent_id=None, nonce=None):
    ...
    db.session.add(row)
    db.session.flush()
    # Push cliente cuando responde el equipo (agent/system) y la conversación
    # tiene customer_id vinculado. No bloquea el flujo si falla el push.
    if sender in ("agent", "system") and conversation.customer_id:
        try:
            from push_service import notify_user
            titulo = "Respuesta del equipo" if sender == "agent" else "Aviso del chat"
            snippet = (clean[:110] + "…") if len(clean) > 110 else clean
            notify_user(
                conversation.customer_id, titulo, snippet,
                url="/chat", tag=f"web_chat_{conversation.id}",
            )
        except Exception:
            current_app.logger.exception("push chat cliente=%s falló", conversation.customer_id)
    return row


# ─── Estado de lectura por sesión (sin migración BD) ──────────────────────
# La marca de "última visita al chat" vive en la cookie de sesión Flask.
# Perdura entre requests pero es local por dispositivo (cada navegador su
# propio contador), coherente con el resto del chat visitante.

_SESSION_LAST_READ_KEY = "wc_last_read_at"


def _visitor_conversation_ids() -> list[int]:
    """IDs de conversaciones asociadas al visitante actual (sesión o cuenta)."""
    from flask_login import current_user
    from models import WebChatConversation
    ids: set[int] = set()
    # Por sesión visitante (siempre)
    try:
        conv = conversation_for_visitor(create=False)
        if conv:
            ids.add(conv.id)
    except Exception:
        pass
    # Por customer_id si está autenticado (cubre multi-dispositivo)
    try:
        if getattr(current_user, "is_authenticated", False) and current_user.id:
            rows = WebChatConversation.query.filter_by(customer_id=current_user.id).all()
...
# TTL sincronizado con routes.public.GUEST_ORDER_TOKEN_TTL_S (24h).
# Duplicado como constante local para evitar import circular.
_GUEST_TOKEN_TTL_S = 24 * 3600


def _ensure_session_token(pedido_id: int) -> str:
    """Devuelve un token de sesión vigente para el pedido; lo crea si falta.

    Necesario para autenticados: sus pedidos pueden verse en un navegador
    donde nunca hicieron checkout, así que  puede no
    tener slot para ese pedido. Sin token la URL de seguimiento devolvería
    403 en .
    """
    slots = session.get("guest_order_tokens", {})
    slot = slots.get(str(pedido_id))
    now = int(datetime.utcnow().timestamp())
    if isinstance(slot, dict):
        exp = int(slot.get("exp") or 0)
        tok = str(slot.get("token") or "")
        if tok and (exp == 0 or exp > now):
            return tok
    elif slot:
        return str(slot)
    token = secrets.token_urlsafe(32)
    slots[str(pedido_id)] = {"token": token, "exp": now + _GUEST_TOKEN_TTL_S}
...
def visitor_orders() -> list[dict]:
    """Pedidos que este navegador (o cliente autenticado) puede consultar/cancelar."""
    slots = session.get("guest_order_tokens", {})
    session_ids = {int(key) for key in slots if str(key).isdigit()}

    # Ampliación: si hay cliente identificado (checkout o push), incluye sus
    # pedidos aunque el navegador no tenga token en sesión. Esto arregla el
    # caso incógnito / dispositivo nuevo / caché borrada.
    customer_id = session.get("push_cliente_id")
    customer_ids: set[int] = set()
    if customer_id:
        try:
            customer_ids = {
                oid for (oid,) in db.session.query(Order.id).filter(
                    Order.cliente_id == int(customer_id),
    ...
    all_ids = session_ids | customer_ids
    if not all_ids:
        return []
    ...
    rows = Order.query.filter(
        Order.id.in_(all_ids),
        Order.estado.in_(("pendiente", "armando", "listo", "en_ruta")),
    ).order_by(Order.creado_en.desc()).limit(10).all()
    result = []
    for order in rows:
        token = _ensure_session_token(order.id)
        if not token:
            continue
        result.append({
            "id": order.id,
